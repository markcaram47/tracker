...
function renderFriends() {
  ...
  if (active.length === 0) {
    ...
    noActive.style.cssText = 'padding:30px 0; grid-column: 1 / -1;';
    ...
    // Append each card directly to el so they flow naturally in the grid
    active.forEach(f => el.appendChild(buildFriendCard(f)));
  }

  // ── Paid / Completed collapsible — spans full width below cards ──
  if (paid.length > 0) {
    const isOpen = STATE.paidFriendsOpen ?? false;

    const accordion = document.createElement('div');
    accordion.className = 'paid-accordion';
    // Force it to span all grid columns so it sits below, not beside, the cards
    accordion.style.cssText = 'margin-top:28px; grid-column: 1 / -1;';
    accordion.innerHTML = `
